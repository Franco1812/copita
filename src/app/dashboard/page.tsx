...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  ...
  const { data: cups } = await supabase.from("cups").select("id,title,slug,status,participant_count").eq("owner_id", userId).order("created_at", { ascending: false });
  const { data: runs } = process.env.SUPABASE_SECRET_KEY
    ? await createAdminClient().from("runs").select("id,cup_id,status,created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(20)
    : { data: null };
  const { data: runCups } = runs?.length
    ? await createAdminClient().from("cups").select("id,title").in("id", [...new Set(runs.map((run) => run.cup_id))])
    : { data: null };
  const cupTitles = new Map(runCups?.map((cup) => [cup.id, cup.title]));
  return <section className="py-12 sm:py-16">
    <div className="flex flex-wrap items-start justify-between gap-4"><div className="flex items-center gap-4">{profile.avatar_url ? <Image src={profile.avatar_url} alt="Tu foto de perfil" width={80} height={80} unoptimized className="aspect-square h-20 w-20 rounded-full object-cover" /> : <div className="flex h-20 w-20 items-center justify-center rounded-full bg-[#dcebe0] text-3xl font-black text-primary" aria-hidden="true">{profile.display_name.charAt(0).toLocaleUpperCase()}</div>}<div><p className="font-bold text-primary">TU ESPACIO</p><h1 className="mt-2 text-4xl font-black">Hola, {profile.display_name}</h1><p className="mt-3 text-muted">Acá vas a encontrar tus Copas.</p></div></div><div className="flex items-center gap-3"><Link href="/profile" className="rounded-xl border border-border bg-surface px-4 py-3 font-semibold">Mi perfil</Link><form action={signOut}><button className="rounded-xl border border-border bg-surface px-4 py-3 font-semibold">Salir</button></form></div></div>
