...
import { addEntries, addEntry, deleteEntry, publishCup, saveCup, updateEntry } from "../actions";
...
export default async function EditCup({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ message?: string }> }) {
  ...
  return <section className="mx-auto max-w-4xl py-10 sm:py-16"><p className="font-bold text-primary">{isDraft ? "BORRADOR" : "COPA PUBLICADA"}</p><h1 className="mt-2 text-4xl font-black tracking-tight">{cup.title}</h1><p className="mt-3 text-muted">{count} / {cup.participant_count} participantes</p>
    ...
    <form action={saveCup.bind(null, id)} className="mt-8 grid gap-5 rounded-2xl border border-border bg-surface p-6 sm:p-8">
      ...
      {isDraft && <div><label htmlFor="participant_count" className="mb-2 block font-bold">Participantes</label><input id="participant_count" name="participant_count" type="number" inputMode="numeric" required min={MIN_PARTICIPANTS} max={MAX_PARTICIPANTS} step={1} defaultValue={cup.participant_count} list="participant-count-options" className="min-h-12 w-40 rounded-xl border border-border bg-white px-4 font-bold" /><datalist id="participant-count-options">{SUGGESTED_PARTICIPANT_COUNTS.map((size) => <option key={size} value={size} />)}</datalist><p className="mt-1 text-sm text-muted">De {MIN_PARTICIPANTS} a {MAX_PARTICIPANTS}. Después de publicar, este número queda fijo.</p></div>}
    ...
    <section id="participantes" className="mt-12"><h2 className="text-2xl font-black">Participantes</h2>
      {!isDraft && <p className="mt-2 text-muted">Podés corregir nombre, descripción, imagen y enlace. No se pueden agregar ni quitar participantes.</p>}
      ...
      {isDraft && count < cup.participant_count && <form action={addEntries.bind(null, id)} className="mt-4 grid gap-4 rounded-2xl border border-border bg-surface p-6">
        <h3 className="text-xl font-bold">Pegar una lista</h3>
        <p className="-mt-2 text-sm text-muted">Un nombre por línea. Entran hasta {cup.participant_count - count} más; después podés editar cada uno para sumarle imagen y descripción.</p>
        <div><label htmlFor="entry-names" className="mb-2 block font-bold">Nombres</label><textarea id="entry-names" name="names" required rows={8} placeholder={"Jimi Hendrix\nDavid Gilmour\nJimmy Page"} className="w-full rounded-xl border border-border px-4 py-3 font-mono text-sm" /></div>
        <button className="min-h-12 rounded-xl bg-primary px-6 font-bold text-white sm:justify-self-start">Agregar lista</button>
      </form>}
