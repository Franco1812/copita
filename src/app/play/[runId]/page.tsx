...
export default async function Play({ params, searchParams }: { params: Promise<{ runId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <section className="py-8 sm:py-12"><div className="mx-auto max-w-5xl text-center"><p className="text-sm font-black uppercase tracking-widest text-primary">{loaded.cup.title}</p><h1 className="mt-3 text-3xl font-black sm:text-4xl">Elegí quién avanza</h1><p className="mt-3 text-muted">{pending.round_number === Math.log2(loaded.cup.participant_count) ? "Final" : `Ronda ${pending.round_number}`} · {numberInRound} de {roundMatches.length}</p><div className="mx-auto mt-6 h-2 max-w-xl overflow-hidden rounded-full bg-[#dce5db]"><div className="h-full rounded-full bg-primary" style={{ width: `${completed / loaded.matches.length * 100}%` }} /></div><p className="mt-2 text-sm text-muted">{completed} de {loaded.matches.length} decisiones</p>
